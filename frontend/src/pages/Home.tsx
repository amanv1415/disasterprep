...
const Home = () => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <img
            ...
            loading="eager"
          />
          <div className="absolute inset-0 bg-gradient-to-r from-secondary/85 via-secondary/65 to-primary/55" />
        </div>
          ...
          <div className="max-w-3xl animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <NavLink to="/dashboard" className="w-full sm:w-auto">
                <Button size="lg" className="gradient-primary text-base sm:text-lg px-6 sm:px-8 py-5 sm:py-6 shadow-strong hover:scale-105 transition-bounce w-full sm:w-auto">
                  <Shield className="mr-2 h-5 w-5" />
                  {t("home.hero.getStarted")}
                </Button>
              </NavLink>
              <NavLink to="/emergency" className="w-full sm:w-auto">
                <Button
                  size="lg"
                  variant="outline"
                  className="bg-emergency text-emergency-foreground border-emergency hover:bg-emergency/90 text-base sm:text-lg px-6 sm:px-8 py-5 sm:py-6 shadow-medium w-full sm:w-auto"
                >
      ...
      {/* Stats Section */}
      <section className="py-10 sm:py-16 bg-card-light">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8 stagger-children">
            {stats.map(({ icon: Icon, value, label }, index) => (
              <div key={index} className="text-center p-3 sm:p-0">
                <div className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 rounded-full gradient-primary mb-3 sm:mb-4">
                  <Icon className="h-6 w-6 sm:h-8 sm:w-8 text-white" />
                </div>
                <div className="text-xl sm:text-2xl md:text-3xl font-display font-bold text-foreground mb-1 sm:mb-2">{value}</div>
            ...
            {features.map(({ icon: Icon, title, description, to }, index) => (
              <Card key={index} className="group hover:shadow-medium transition-smooth cursor-pointer">
                <CardContent className="p-5 sm:p-6">
                  <NavLink to={to}>
                    <div className="inline-flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-lg gradient-primary mb-3 sm:mb-4 group-hover:scale-110 transition-bounce">
                      <Icon className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                    </div>
                    <h3 className="text-lg sm:text-xl font-display font-semibold mb-2 sm:mb-3">{title}</h3>
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <NavLink to="/login">
            <Button
              size="lg"
              variant="secondary"
              className="text-base sm:text-lg px-6 sm:px-8 py-5 sm:py-6 shadow-strong hover:scale-105 transition-bounce"
            >
